...
for i in range(total):
    ...
    # prediction_orig_list=dense_model.predict(X.reshape(1,28,28,1))[0]
    # prediction_orig=np.argmax(prediction_orig_list)
    
    label = np.argmax(Y)
    # if prediction_orig == label:
    #     counter1 += 1
    dense_out = [(int(x)+1e18)%p for x in dense_out]
    prediction_circiut = np.argmax(dense_out)
    if prediction_circiut == label:
        counter2 += 1
...
# print("Original model accuracy: ", (counter1/total)*100)
print("Circuit model accuracy: ", (counter2/total)*100)
