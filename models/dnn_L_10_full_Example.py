...
model = models.Sequential([
    ...
    layers.Dense(10, activation='relu'),     # 3th hidden layer
    layers.Dense(10, activation='relu'),     # 4th hidden layer
    layers.Dense(10, activation='relu'),     # 5th hidden layer
    layers.Dense(10, activation='relu'),     # 6th hidden layer
    layers.Dense(10, activation='relu'),     # 7th hidden layer
    layers.Dense(10, activation='relu'),     # 8th hidden layer
    layers.Dense(10, activation='relu'),     # 9th hidden layer
    layers.Dense(10, activation='relu'),     # 10th hidden layer
    # next 10 layers
    layers.Dense(10, activation='relu'),     # First hidden layer
    layers.Dense(10, activation='relu'),     # Second hidden layer
    layers.Dense(10, activation='relu'),     # 3th hidden layer
...
for i in range(1,num_hidden_layers+1):
    ...
    dense_weights_hi = [[int(model.layers[i].weights[0][k][j]*1e18) for j in range(10)] for k in range(10)]
    dense_bias_hi = [int(model.layers[i].weights[1][j]*1e36) for j in range(10)]
    _, dense_weights_hi, dense_bias_hi, dense_hi_out, dense_remainder_hi = DenseInt(10, 10, 10**18, dense_input_next, dense_weights_hi, dense_bias_hi)
    relu_in_hi = [int(dense_hi_out[j]) for j in range(10)]
    relu_out_hi = [str(relu_in_hi[j]) if relu_in_hi[j] < p//2 else '0' for j in range(10)]
...
# dense_input_h3=[int(x) for x in relu_out_h2]

# dense_weights_h3 = [[int(model.layers[3].weights[0][i][j]*1e18) for j in range(10)] for i in range(10)]
# dense_bias_h3 = [int(model.layers[3].weights[1][i]*1e36) for i in range(10)]
# _, dense_weights_h3, dense_bias_h3, dense_h3_out, dense_remainder_h3 = DenseInt(10, 10, 10**18, dense_input_h3, dense_weights_h3, dense_bias_h3)

# relu_in_h3 = [int(dense_h3_out[i]) for i in range(10)]
# relu_out_h3 = [str(relu_in_h3[i]) if relu_in_h3[i] < p//2 else '0' for i in range(10)]
# dense_input_h4=[int(x) for x in relu_out_h3]

# dense_weights_h4 = [[int(model.layers[4].weights[0][i][j]*1e18) for j in range(10)] for i in range(10)]
# dense_bias_h4 = [int(model.layers[4].weights[1][i]*1e36) for i in range(10)]
# _, dense_weights_h4, dense_bias_h4, dense_h4_out, dense_remainder_h4 = DenseInt(10, 10, 10**18, dense_input_h4, dense_weights_h4, dense_bias_h4)

# relu_in_h4 = [int(dense_h4_out[i]) for i in range(10)]
# relu_out_h4 = [str(relu_in_h4[i]) if relu_in_h4[i] < p//2 else '0' for i in range(10)]
# dense_input_out=[int(x) for x in relu_out_h4]

out_layer_index = num_hidden_layers+1
dense_weights_out = [[int(model.layers[out_layer_index].weights[0][i][j]*1e18) for j in range(10)] for i in range(10)]
dense_bias_out = [int(model.layers[out_layer_index].weights[1][i]*1e36) for i in range(10)]
_, dense_weights_out, dense_bias_out, dense_out_out, dense_remainder_out = DenseInt(10, 10, 10**18, dense_input_next, dense_weights_out, dense_bias_out)
...
outPutFile="./data/circiut_inputs/mnist_input_"+str(num_hidden_layers)+"_10_dig"+str(Y)+".json"
